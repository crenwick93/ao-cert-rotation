...
# 1. Renew both certs to full 90-day validity
echo "  [1/4] Renewing both certificates to 90 days..."
ansible-playbook -i "${INVENTORY}" "${REPO_ROOT}/setup/playbooks/provision_vm.yml" \
  --start-at-task="Issue initial certificate from Vault" \
  --tags untagged 2>/dev/null | tail -3
ansible-playbook -i "${INVENTORY}" "${REPO_ROOT}/setup/playbooks/provision_tomcat.yml" \
  --start-at-task="Pull CA certificate from Vault" 2>/dev/null | tail -3
echo "  Both certs renewed (~90 days)"
echo ""

# 2. Reset Splunk alert
echo "  [2/4] Resetting Splunk alert..."
SPLUNK_AUTH="admin:${SPLUNK_ADMIN_PASSWORD}"
ssh -i "${KEY}" -o StrictHostKeyChecking=no ec2-user@${HOST_IP} \
  "sudo curl -sk -X DELETE 'https://localhost:8089/servicesNS/admin/search/saved/searches/Certificate%20Expiry%20Alert' -u '${SPLUNK_AUTH}' -o /dev/null 2>/dev/null; \
   sleep 1; \
   sudo curl -sk -X POST 'https://localhost:8089/servicesNS/admin/search/saved/searches' -u '${SPLUNK_AUTH}' \
     ...
     -d 'dispatch.earliest_time=-10m&dispatch.latest_time=now' -o /dev/null 2>/dev/null" 2>/dev/null
echo "  Splunk alert reset (clean suppression)"
echo ""
